...
(def (function e) select-survivors (population ratio &key (maximize nil) (get-fit-fn #'get-fit))
...
(def (function ei) select-the-best (population &key (maximize nil) (get-fit-fn #'get-fit))
  ...
  (let ((var-names (mapcar #'car genome)))
    ...
    `(flet ((fit (,@var-names)
       ...
       (let* ((genome (make-genome-by-definition ',genome
         ...
         (with-simple-restart (stop-algorithm "Stop Genetic Algorithm evaluation and return the best result.")
           (loop for -iteration- from 1 upto ,max-iterations
                 for survivors = (arrows:-> -population-
                                   ...
                                   (,calculate-fit #'fit)
                                   (select-survivors ,survive-ratio :maximize ,maximize :get-fit-fn ,get-fit-fn))
         ...
         (arrows:-> -population-
           (remove-if-null-fit)
           (select-the-best :maximize ,maximize :get-fit-fn ,get-fit-fn))))))
